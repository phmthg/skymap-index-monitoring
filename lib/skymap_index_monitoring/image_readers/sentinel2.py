...
class S2L1CReader(MultiBandReader):
    """AWS Public Dataset Sentinel 2 L1C image_readers.
...
class S2L2AReader(S2L1CReader):
    """AWS Public Dataset Sentinel 2 L2A image_readers.
...
class S2L2ACOGReader(MultiBandReader):
    """AWS Public Dataset Sentinel 2 L2A COGS image_readers.
    ...
    def __attrs_post_init__(self):
        ...
        self.bands = tuple(
            [info["href"].split("/")[-1][0:-4] for band, info in self.stac_item["assets"].items() if re.match("B[0-9A]{2}.tif", info["href"].split("/")[-1])]
        )
